"""Q2 Streamlit 배포용 앱 (kaggle_M 4강 패턴 참고). 실행: streamlit run sunspots_app.py"""
...
def plot_advanced_sunspot_visualizations(df, sunactivity_col="SUNACTIVITY"):
    ...
    if len(data) > 0:
        ...
        axs[0, 1].hist(
            data, bins=30, density=True, alpha=0.6, color="gray", label="Histogram"
        )
    ...
    if len(years_clean) > 1:
        axs[1, 1].scatter(
            years_clean, sun_activity_clean, s=10, alpha=0.5, label="Data Points"
        )
        coef = np.polyfit(years_clean, sun_activity_clean, 1)
        trend = np.poly1d(coef)
        axs[1, 1].plot(
            years_clean, trend(years_clean), color="red", linewidth=2, label="Trend Line"
        )
...
try:
    ...
    st.info(
        "데이터 파일의 구조를 확인해주세요. 'data/sunspots.csv' 파일이 존재하고 "
        "'YEAR'와 'SUNACTIVITY' 컬럼이 있어야 합니다."
    )
